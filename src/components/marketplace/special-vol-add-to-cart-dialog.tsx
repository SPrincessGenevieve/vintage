...
export default function SpecialVolAddToCartDialog({
  market_value,
  lwin11,
  investment_id,
  case_size,
  ...
  const handleAddToCart = async (e: any) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        const notifCount = Array.isArray(response_notif.data)
  ...
  const handleAddToCartHigherPrice = async (e: any) => {
    e.preventDefault();
    // setOpen(false);
    setLoader("");
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        const itemCount = Array.isArray(response.data)
          ? response.data.length
          : 0;
        console.log("COUNNNNNNT: ", itemCount);
        setUserDetails({
          cartCount: itemCount,
          notification_count: response_notif.data.length,
        });
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            Price: <span className="text-red-700">£{price}</span>
              ...
              <div className="flex justify-between gap-4">
                ...
                <Button
                  disabled={isDisabled}
                  onClick={handleAddToCartHigherPrice}
