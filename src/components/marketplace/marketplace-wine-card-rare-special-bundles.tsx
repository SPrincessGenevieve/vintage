...
export default function MarketplaceWineCardRareSpecialBundles({
  item,
}: {
  ...
  const bundle_parent = item.basket_details;
  const bundle_details = item.basket_items;

  const displayValue = (value: string | undefined) => {
    return value === "NA" ? "" : value || "";
  };

  const market_value_total = bundle_details.reduce(
          ...
          <div className="relative border-gray-200 flex h-full w-full border rounded-2xl items-center justify-center">
            <div className="flex w-full h-full items-center justify-center p-10">
              <Image
                width={400}
                height={400}
                src={bundle_parent.image || "/fallback-image.jpg"}
                alt="card"
      ...
      <div className="flex w-full relative h-auto items-center flex-col px-5">
        <div className="mt-6 port-desc">
          <h1 className="text-[16px] font-semibold">
            {displayValue(bundle_parent?.name) || "None"}
          </h1>

          <div>
            <p className="text-justify text-muted-foreground text-[14px]">
              {displayValue(bundle_parent.winery) || "None"} <br />
            </p>
          </div>
              ...
              <DialogContent className="w-[90%] max-h-[90%] h-auto  overflow-y-auto flex flex-col">
                ...
                <div className="h-full">
                  <p className="text-[14px]  text-justify">
                    {displayValue(bundle_parent?.region) ||
                      "No further details..."}
                  </p>
                </div>
              </DialogContent>
            ...
            <Dialog>
              ...
              <DialogContent className="w-[90%] max-h-[90%] h-auto overflow-y-auto flex flex-col">
                ...
                <div className="h-full">
                  <p className="text-[14px] text-justify">
                    {
                      // displayValue(bundle_parent.grape_variety) ||
                      "No further details..."
                    }
                  </p>
                </div>
            ...
            <div className=" h-full flex flex-col justify-between">
              ...
              <div className="ml-auto flex items-end gap-4 bgn-group-con">
                ...
                <BuyNowSpecialBundlesDialog
                  ...
                  message={message}
                  investment_id={bundle_parent.id}
                  market_value={String(item.market_value)}
                  lwin11={""}
                  case_size={Number(item.case_size)}
