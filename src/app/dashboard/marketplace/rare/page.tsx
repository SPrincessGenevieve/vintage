...
function RarePage() {
  ...
  const { setUserDetails, marketplace_invest, sessionkey } = useUserContext();
  const [pageList, setPageList] = useState(1);
  const [currentPageRare, setCurrentPageRare] = useState(() => {
    return Number(localStorage.getItem("currentPageRare")) || 1;
  });
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const responseMarketPlace = await axios.get(`${LinkApi.href}/rare/`, {
          headers: {
        ...
        setUserDetails({
          marketplace_invest: responseMarketPlace.data.results,
        });
          ...
          <div className="ml-auto flex flex-wrap items-start gap-2 w-full">
            <div className="ml-auto flex flex-wrap items-center gap-2 w-full">
              {WineFilters.map((item, index) => (
                <Button
                  variant={item.name === "Trending Wines" ? "default" : "ghost"}
                  className={`text-[10px] rounded-full ${
                    item.name === "Trending Wines" ? "" : "border"
                  }`}
...
export default withAuth(RarePage);
