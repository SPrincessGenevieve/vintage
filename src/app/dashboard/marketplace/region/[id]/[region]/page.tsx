...
import MarketplaceWineCardRegion from "@/components/marketplace/marketplace-wine-card-region";
import MarketHeaderRegion from "@/components/marketplace/market-header-region";
import WineChartRegion from "@/components/marketplace/wine-chart-card-region";
...
function RegionWine() {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!id || !sessionkey || !region) return;
    ...
    <div className="bg-[#FCFCFC] flex flex-col w-full h-auto min-h-full">
      ...
      <div className="flex flex-col bg-[#FCFCFC] w-full h-full">
        ...
        <div className="px-2 pb-2 port-charts-cont w-full h-auto">
          <WineChartRegion region={region} parent={dataParent} item={dataVintage[yearSelect] || dataVintage[0]} />
        </div>
      </div>
    </div>
  );
}
export default withAuth(RegionWine);
