...
import SpecialBundlesCard from "@/components/marketplace/special-bundles-card";

function SpecialBundlePage() {
  const router = useRouter();
  const { setUserDetails, special_bundle_list, sessionkey } = useUserContext();
  const [pageList, setPageList] = useState(1);
  const [currentPageSpecialBundle, setCurrentPageSpecialBundle] = useState(
    () => {
      return Number(localStorage.getItem("currentPageSpecialBundle")) || 1;
    }
  );
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const special_bundle_data = await axios.get(
          `${LinkApi.href}/special-bundles/`,
          {
            headers: {
        ...
        console.log("ASSORTMENT DATA: ", special_bundle_data.data.results);
        setUserDetails({
          special_bundle_list: special_bundle_data.data.results,
        });
        setPageList(special_bundle_data.data.total_pages);
      } catch (error) {
  ...
  const handleNext = async (index: number) => {
    ...
    setCurrentPageSpecialBundle(index);
    try {
      const response = await axios.get(
        `${LinkApi.href}/special-bundles/?page=${index}`,
        {
          headers: {
            Authorization: authHeader,
            "Content-Type": "application/json",
      ...
      setUserDetails({ special_bundle_list: response.data.results });
    } catch (error) {
      setVisible(false);
    ...
    <Suspense>
      ...
      <div className="relative flex h-full w-full flex-col px-4">
          ...
          <div className="ml-auto flex flex-wrap items-start gap-2 w-full">
            <div className="ml-auto flex flex-wrap items-center gap-2 w-full">
              {WineFilters.map((item, index) => (
                <Button
                  variant={
                    item.name === "Special Bundles" ? "default" : "ghost"
                  }
                  className={`text-[10px] rounded-full ${
                    item.name === "Special Bundles" ? "" : "border"
                  }`}
        ...
        <div className="flex gap-2 absolute w-full bottom-1">
          <ItemsPagination
            totalPages={pageList}
            currentPage={currentPageSpecialBundle}
...
export default withAuth(SpecialBundlePage);
