...
function WineSpecialBundles() {
    ...
    const fetchUserData = async () => {
      ...
      try {
        const response = await axios.get(
          `${apiUrl}/api/wine/special-bundles/${id}/`,
          {
            headers: {
        ...
        setUserDetails({
          special_bundle_item_list: userDataVintage,
          special_bundle_list: [userDataParent],
        });
      } catch (error) {
  ...
  console.log("DETAILS LIST: ", special_bundle_list);
  console.log("DATA CHILDREN: ", dataVintage);
  const bottle_size = special_bundle_item_list[0]?.basket_bottle_size || "";

  return (
    <div className="bg-[#FCFCFC] flex flex-col w-full h-full">
      <SpecialBundleChildHeader name={special_bundle_list[0]?.name || ""} />
      <MarketHeader name={special_bundle_list[0]?.name || ""}></MarketHeader>
      <div id="main-div" className="flex flex-col bg-[#FCFCFC] w-full h-full ">
        <div id="parent-wines" className="px-2 py-2 w-full h-auto flex">
          <MarketplaceWineCardSpecialBundles
            parent={special_bundle_list[0]}
            vintage={special_bundle_item_list}
          />
        </div>
        <div className="px-2 py-2 w-full h-auto">
          <div className="w-full h-auto bg-[white] p-2 rounded-2xl flex gap-4">
            <div className="#FCFCFC h-[350px] p-2 w-[90px] bg-white overflow-x-auto flex gap-2 flex-grow border rounded-2xl">
              {special_bundle_item_list.map((item) => (
                <Link
                  key={item.id}
                  ...
                  <div>
                    ...
                    <div className="text-[10px] font-light text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>Price: </span>
                        <span>£{item.wine_vintage.market_value.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span>Bottle Size: </span>
                        <span>
                          {special_bundle_list[0]?.case_size || ""}x
                          {bottle_size === "0750"
                            ? "75cl"
                            : bottle_size === "1500"
                            ? "150cl"
