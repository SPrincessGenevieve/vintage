...
import AssortmentCard from "@/components/marketplace/assortment-card";

function AssortmentPage() {
  const router = useRouter();
  const { setUserDetails, assortment_list, sessionkey } = useUserContext();
  const [pageList, setPageList] = useState(1);
  const [currentPageAssortment, setCurrentPageAssortment] = useState(() => {
    return Number(localStorage.getItem("currentPageAssortment")) || 1;
  });
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const assortment_data = await axios.get(`${LinkApi.href}/assortment/`, {
          headers: {
            Authorization: authHeader,
            "Content-Type": "application/json",
          },
        });
        console.log("ASSORTMENT DATA: ", assortment_data.data.results);
        setUserDetails({
          assortment_list: assortment_data.data.results,
        });
        setPageList(assortment_data.data.total_pages);
      } catch (error) {
  ...
  const handleNext = async (index: number) => {
    ...
    setCurrentPageAssortment(index);

    try {
      const response = await axios.get(
        `${LinkApi.href}/assortment/?page=${index}`,
        {
          headers: {
            Authorization: authHeader,
            "Content-Type": "application/json",
      ...
      setUserDetails({ assortment_list: response.data.results });
    } catch (error) {
      setVisible(false);
    ...
    <Suspense>
      ...
      <div className="relative flex h-full w-full flex-col  px-4">
          ...
          <div className="ml-auto flex flex-wrap items-start gap-2 w-full">
            <div className="ml-auto flex flex-wrap items-center gap-2 w-full">
              {WineFilters.map((item, index) => (
                <Button
                  variant={
                    item.name === "Assortment Cases" ? "default" : "ghost"
                  }
                  className={`text-[10px] rounded-full ${
                    item.name === "Assortment Cases" ? "" : "border"
                  }`}
        ...
        <div className="flex gap-2 absolute w-full bottom-1">
          <ItemsPagination
            totalPages={pageList}
            currentPage={currentPageAssortment}
...
export default withAuth(AssortmentPage);
